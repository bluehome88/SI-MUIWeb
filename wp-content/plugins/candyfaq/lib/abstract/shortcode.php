...
abstract class KST_Shortcode {
	...
	final public function get_html($atts, $content = '') {
		ob_start();
		?><div class="<?php esc_attr_e(CSS_CLASS); ?>shortcode-container"><?php
	...
	final protected function vc_params_adapter($custom_params = array()) {
		...
		if ($this->has_content) {
			array_push($params, array(
				...
				"holder" => "div",
				"class" => "",
				"heading" => __("Content", 'candy-faq'),
				"param_name" => "content",
				"value" => __("Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.", 'candy-faq'),
				"description" => __("Edit shortcode content.", 'candy-faq')
			));
		}
	...
	/**
	 * Removes params hidden by dependencies
	 * @param $params
	 *
	 * @return array
	 */
	final protected function clean_params($params) {
		$options = $this->get_options_by_id();
		$visible = array();

		$params = $this->normalize_values($params);
		...
		foreach($params as $id => $value) {
			...
			if (!isset($option['dependency']) || $this->check_dep($option['dependency'], $params)) {
				$visible[$id] = $value;
			}
		}
	...
	/**
	 * Checks visibility by dep config
	 * @param $config
	 * @param $params
	 *
	 * @return bool
	 */
	final protected function check_dep($config, $params) {
		switch ($config['type']) {
			case 'EQ':
				return $params[$config['target']] === $config['value'];
	...
	final protected function normalize_values($settings) {
		foreach($settings as $id => $value) {
			...
			} else if ((int)$value > 0 && in_array($id, array('search_container_image_bg'), true)) {;
				$settings[$id] = wp_get_attachment_url( $value );
			}
		}
	 ...
	 * @return mixed
	 */
	final protected function get_options_by_id() {
		return array_reduce(Options::get_non_ui_options($this->get_options()), function($options, $option) {
	...
	final protected function get_defaults() {
		return array_reduce(Options::get_non_ui_options($this->get_options()),
