	...
	public function load_articles() {
		...
		if (!empty($this->articles)) {
			...
			foreach($this->articles as $article):
				$this->views_count += (int) get_post_meta($article->ID, VIEW_FIELD, true);
				$this->likes_count += (int) get_post_meta($article->ID, LIKE_FIELD, true);
				$this->dislikes_count += (int) get_post_meta($article->ID, DISLIKE_FIELD, true);

				$views_meta = get_post_meta($article->ID, VIEW_META_FIELD, true);
				$views_meta = $views_meta ? json_decode($views_meta, true) : array();

				$likes_meta = get_post_meta($article->ID, LIKE_META_FIELD, true);
				$likes_meta = $likes_meta ? json_decode($likes_meta, true) : array();

				$dislikes_meta = get_post_meta($article->ID, DISLIKE_META_FIELD, true);
		...
		$this->topics = get_terms( FAQ_CPT_CATEGORY, array(
			'hide_empty' => false,
		));
		...
		if ( ! empty( $articles ) ):
			$result = array_map(function($article) {
				return array(
					...
					'title' => $article->post_title,
					'link' => get_the_permalink($article->ID),
					'views' => (int) get_post_meta($article->ID, VIEW_FIELD, true),
				);
			}, $articles);
		endif;
		...
		if ( ! empty( $articles ) ):
			$result = array_map(function($article) {
				return array(
					...
					'title' => $article->post_title,
					'link' => get_the_permalink($article->ID),
					'likes' => (int) get_post_meta($article->ID, LIKE_FIELD, true),
				);
			}, $articles);
		endif;
		...
		if ( ! empty( $articles ) ):
			$result = array_map(function($article) {
				return array(
					...
					'title' => $article->post_title,
					'link' => get_the_permalink($article->ID),
					'dislikes' => (int) get_post_meta($article->ID, DISLIKE_FIELD, true),
				);
			}, $articles);
		endif;
