...
(function($) {
    ...
    function onSaveSettings(e) {
        ...
        ui.fetch({
            action: ajaxPrefix + '_save_settings',
    ...
    function onResetSettings(e) {
        ...
        ui.fetch({
            action: ajaxPrefix + '_reset_settings'
        }).always(function(response) {
    ...
    function onDemoImport(e) {
        ...
        ui.fetch({
            action: ajaxPrefix + '_demo_import',
    ...
    function onSkipDemoImport(e) {
        ...
        ui.fetch({
            action: ajaxPrefix + '_skip_demo_import'
        }).done(function() {
    ...
    function setDemoImportControls() {
        ...
        $container.on('click', '.fn-import-entities-remove', function(e) {
            ...
            if (confirm('Remove selected ' + groupType + '?')) {
                ...
                ui.fetch({
                    action: ajaxPrefix + '_remove_import_entities',
                    ids: selected,
                    type: groupType
                }).done(function(response) {
        ...
        $('.fn-demo-import-remove-all').on('click', function(e) {
            ...
            if (confirm('Remove all imported data?')) {
                ...
                ui.fetch({
                    action: ajaxPrefix + '_remove_all_import_entities'
                }).done(function(response) {
                    if (response.status === 0) {
    ...
    function init() {
        ...
        ui.setupPresetLoader($form, 'mkb_option_');
